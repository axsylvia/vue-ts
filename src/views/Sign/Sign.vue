...
// 监听初始与更新:watchEffect
// 统计结果与响应式数据变化:detailValue
// 描述列表数据渲染
watchEffect((reset) => {
  // console.log("month.value", month.value);
  // console.log("signsInfos.value.detail", signsInfos.value.detail);
  ...
  // detailMonth是个对象集合，用for循环
  // switch多重判断
  for (const attr in detailMonth) {
    console.log("attr", attr);
    console.log("detailMonth[attr]", detailMonth[attr]);
...
// 日历数据渲染
// 后端接口`/signs/time`获取数据后，需要把打卡信息渲染到日历中
